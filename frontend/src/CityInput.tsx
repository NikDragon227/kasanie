...
export function CityInput({ name = 'city', defaultValue = '', required = false }: { name?: string; defaultValue?: string; required?: boolean }) {
  ...
  return <div className="city-picker">
    <input name={name} value={value} required={required} autoComplete="address-level2" placeholder="Начните вводить город"
      ...
      onFocus={() => setOpen(true)} onChange={event => { setValue(event.target.value); setOpen(true) }}
    ...
    {open && value.trim().length >= 2 && <ul id={listId} className="city-suggestions" role="listbox">
      {options.length > 0 ? options.map(option => <li key={`${option.city}-${option.region}`}>
        <button type="button" onMouseDown={event => event.preventDefault()} onClick={() => { setValue(option.city); setOpen(false) }}>
