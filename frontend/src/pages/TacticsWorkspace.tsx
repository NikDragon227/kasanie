...
function Pitch({ slots, lineup, playerById, selectedPlayerId, onDrop, onClick }: { slots: Slot[]; lineup: Lineup; playerById: (id?: number) => Player | undefined; selectedPlayerId: number | null; onDrop: (slotId: string) => (event: React.DragEvent<HTMLButtonElement>) => void; onClick: (slotId: string) => () => void }) {
  return <><span className="pitch-goal pitch-goal-top" aria-hidden="true" /><span className="pitch-goal pitch-goal-bottom" aria-hidden="true" />{slots.map(slot => { const player = playerById(lineup[slot.id]); return <button type="button" draggable={Boolean(player)} key={slot.id} className={selectedPlayerId && !player ? 'ready' : ''} style={{ left: `${slot.x}%`, top: `${slot.y}%` }} onDragStart={event => { if (!player) return; event.dataTransfer.setData('playerId', String(player.playerId)); event.dataTransfer.setData('slotId', slot.id) }} onDragOver={event => event.preventDefault()} onDrop={onDrop(slot.id)} onClick={onClick(slot.id)}><small>{slot.label}</small><strong>{player ? `${player.shirtNumber || '—'} · ${player.lastName}` : '＋'}</strong></button> })}</>
}
