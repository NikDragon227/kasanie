...
function PublicHeader() {
  ...
  return <header className="nearby-header"><Link className="brand" to="/" aria-label="Касание — главная"><span className={`brand-emblem${isDiscoveryHome ? ' brand-emblem-outlined' : ''}`}><img src={isDiscoveryHome ? '/brand/kasanie-logo-official.png' : '/brand/kasanie-mark-light.svg'} alt="" /></span><span><strong>КАСАНИЕ</strong><small>спортивная платформа</small></span></Link><nav><Link to={isOrganizer ? '/organizer/activities' : '/register-organizer'}>Организаторам</Link>{isDiscoveryHome && <Link className="button nearby-develop-button" to="/join">Развиваться</Link>}{user
