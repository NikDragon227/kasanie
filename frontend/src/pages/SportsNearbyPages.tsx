...
export function OrganizerActivitiesPage() {
  ...
  const startEditing = (activity: Activity) => {
    setEditing(activity); setSelectedSportId(sports.find(sport => sport.slug === activity.sportSlug)?.id ?? null); setVenueChoice(activity.venue.id.toString()); setMeetingPoint(null); setVenueDraft(emptyVenueDraft); setMessage(null); setFormVersion(value => value + 1)
    document.querySelector('.organizer-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  }
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      form.reset(); setMessage({ text: editing ? 'Изменения сохранены.' : 'Активность опубликована.', ok: true }); clearEditor(); await reload()
  ...
  const deleteActivity = async (activity: Activity) => {
    ...
    try {
      ...
      if (editing?.id === activity.id) clearEditor()
      if (participantActivity?.id === activity.id) { setParticipantActivity(null); setParticipants(null) }
  ...
  const openParticipants = async (activity: Activity) => {
    setParticipantActivity(activity); setParticipantLoading(true); setConfirmRemovalId(null); setMessage(null)
