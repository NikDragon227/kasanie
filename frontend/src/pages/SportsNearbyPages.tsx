...
function PublicHeader() {
  ...
  return <header className="nearby-header"><Link className={`brand${isDiscoveryHome ? ' brand-official' : ''}`} to="/" aria-label="Касание — главная">{isDiscoveryHome
    ? <span className="brand-official-lockup" aria-hidden="true"><span className="brand-official-mark"><img src="/brand/kasanie-logo-official.png" alt="" /></span><span className="brand-official-word"><img src="/brand/kasanie-logo-official.png" alt="" /></span></span>
    : <><span className="brand-emblem"><img src="/brand/kasanie-mark-light.svg" alt="" /></span><span><strong>КАСАНИЕ</strong><small>спортивная платформа</small></span></>}</Link><nav><Link to={isOrganizer ? '/organizer/activities' : '/register-organizer'}>Организаторам</Link>{isDiscoveryHome && <Link className="button nearby-develop-button" to="/join">Развиваться</Link>}{user
