...
import { ApiError, api, post, postForm, put, remove } from '../api'
...
const activityPlaceholderBySport: Record<string, string> = {
  football: '/brand/activity-placeholders/football.png',
  basketball: '/brand/activity-placeholders/basketball.png',
  volleyball: '/brand/activity-placeholders/volleyball.png',
  running: '/brand/activity-placeholders/running.png',
  hockey: '/brand/activity-placeholders/hockey.png',
  tennis: '/brand/activity-placeholders/tennis.png',
  badminton: '/brand/activity-placeholders/badminton.png',
  workout: '/brand/activity-placeholders/workout.png',
  'adaptive-sport': '/brand/activity-placeholders/adaptive-sport.png'
}

const activityCover = (activity: Pick<Activity, 'coverImageUrl' | 'sportSlug'>) =>
  activity.coverImageUrl || activityPlaceholderBySport[activity.sportSlug] || activityPlaceholderBySport.football
...
export function SportsNearbyPage() {
  ...
  const recentlyViewedIds = new Set(recentlyViewed.map(item => item.activity.id))
  const cityShelves = Array.from(browseItems.filter(item => !recentlyViewedIds.has(item.activity.id)).reduce((groups, item) => {
  ...
  return <div className="nearby-page search-discovery"><PublicHeader /><main>
    ...
    {isBrowseHome ? <section className="discovery-shelves" aria-label="Подборки активностей">
      ...
      {cityShelves.map(([city, items]) => <DiscoveryShelf key={city} title={cityShelves.length === 1 ? `Актуальные события · ${city}` : `Активности: ${city}`} items={items} />)}
...
export function OrganizerActivitiesPage() {
  ...
  useEffect(() => {
    if (!coverFile || typeof URL.createObjectURL !== 'function') { setCoverPreviewUrl(null); return }
  ...
  const clearEditor = () => {
    setEditing(null); setTemplate(null); setSelectedSportId(sports[0]?.id ?? null); setMeetingPoint(null); setVenueDraft(emptyVenueDraft); setVenueChoice(venues[0]?.id.toString() ?? 'new-map'); setCoverFile(null); setRemoveCover(false); setFormVersion(value => value + 1)
  }
  ...
  const startEditing = (activity: Activity) => {
    setEditing(activity); setTemplate(null); setSelectedSportId(sports.find(sport => sport.slug === activity.sportSlug)?.id ?? null); setVenueChoice(activity.venue.id.toString()); setMeetingPoint(null); setVenueDraft(emptyVenueDraft); setCoverFile(null); setRemoveCover(false); setMessage(null); setFormVersion(value => value + 1); setView('form')
  }

  const startRepeating = (activity: Activity) => {
    setEditing(null); setTemplate(activity); setSelectedSportId(sports.find(sport => sport.slug === activity.sportSlug)?.id ?? null); setVenueChoice(activity.venue.id.toString()); setMeetingPoint(null); setVenueDraft(emptyVenueDraft); setCoverFile(null); setRemoveCover(false); setMessage(null); setConfirmDeleteId(null); setFormVersion(value => value + 1); setView('form')
  }
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editing) { await put(`/api/organizer/activities/${editing.id}`, payload); activityId = editing.id }
      else { activityId = (await post<{ id: number }>('/api/organizer/activities/', payload)).id }
      ...
      try {
        if (coverFile) {
          ...
          await postForm(`/api/organizer/activities/${activityId}/cover`, cover)
        } else if (editing && removeCover && editing.coverImageUrl) {
          await remove(`/api/organizer/activities/${activityId}/cover`)
        }
      } catch (coverError) {
        coverWarning = coverError instanceof Error ? coverError.message : 'Фотография не загрузилась.'
      }
      if (!editing) await post(`/api/organizer/activities/${activityId}/publish`)
      form.reset()
      setMessage({ text: coverWarning ? `${editing ? 'Изменения сохранены' : 'Активность опубликована'}, но фото не обновилось: ${coverWarning}` : editing ? 'Изменения сохранены.' : 'Активность опубликована.', ok: !coverWarning })
      clearEditor(); setView('list'); await reload()
...
export function PublicActivityPage() {
  ...
  return <div className="nearby-page">
    ...
    <main className="activity-detail">
      ...
      <div className="activity-detail-cover"><img src={activityCover(activity)} alt={`${activity.sport}: ${activity.title}`} /></div>
