...
const audienceCards = [
  { image: 'child-parent', title: 'Дети и родители', copy: 'Секции и занятия для ребёнка. Получайте обратную связь и находите лучшие занятия рядом с вами.', link: '/register-parent' },
  { image: 'player', title: 'Игроки', copy: 'Находите игры и тренировки, развивайте навыки и достигайте своих спортивных целей.', link: '/register' },
  { image: 'coach', title: 'Тренеры', copy: 'Планируйте тренировки, управляйте командами и отслеживайте прогресс своих игроков.', link: '/register-coach' },
  { image: 'school', title: 'Академии и секции', copy: 'Управляйте группами, расписанием и взаимодействием с игроками и их родителями.', link: '/login' }
] as const
...
function Header() {
  ...
  return <header className="home-header"><Brand /><button className="home-menu" type="button" aria-expanded={open} aria-controls="home-nav" onClick={() => setOpen(value => !value)}><span /><span /><span /><b>Меню</b></button><nav id="home-nav" className={open ? 'open' : ''} aria-label="Навигация по главной странице"><a href="#top" onClick={() => setOpen(false)}>Главная</a><a href="#families" onClick={() => setOpen(false)}>Для родителей</a><a href="#coaches" onClick={() => setOpen(false)}>Для тренеров</a></nav></header>
}
...
function Footer() {
  return <footer className="home-footer"><div className="home-footer-main"><div className="home-footer-brand"><Brand /><p>Платформа для спорта и развития.<br />Объединяем игроков, команды и возможности.</p></div><div className="home-footer-links"><a href="#top">Главная</a><a href="#families">Для родителей</a><a href="#coaches">Для тренеров</a></div><div className="home-footer-contact"><span>Свяжитесь с нами</span>{/* TODO: заменить на рабочий ящик, когда заведём почту на prokasanie.ru */}<a href="mailto:hello@prokasanie.ru"><HomeIcon name="mail" />hello@prokasanie.ru</a></div><div className="home-footer-social"><span>Мы в соцсетях</span><div><a href="#social-vk" aria-label="ВКонтакте"><HomeIcon name="vk" /></a><a href="#social-telegram" aria-label="Telegram"><HomeIcon name="telegram" /></a><a href="#social-instagram" aria-label="Instagram"><HomeIcon name="instagram" /></a></div></div></div><div className="home-footer-bottom"><span>© 2026 КАСАНИЕ</span><div><a href="#privacy">Политика конфиденциальности</a><i /><a href="#terms">Пользовательское соглашение</a></div></div></footer>
}
