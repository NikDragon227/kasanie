...
- общая статистика платформы доступна только администратору и не публикуется в пользовательских кабинетах;
...
- профиль развития доступен самому игроку, назначенному через активную команду тренеру и связанному родителю; ресурсная проверка выполняется до расчёта статистики, прямой URL чужого игрока возвращает 403;
...
10. Проверить, что `/api/analytics/overview` возвращает 404, а сводка `/api/admin/summary` доступна только администратору.
