...
Командный журнал фиксирует снимок состава при создании занятия. Посещаемость и результаты хранятся отдельно от текущего состава команды, поэтому переход игрока не меняет историю. Завершение требует статуса посещения для каждого игрока и двух отметок по каждому упражнению у присутствующих: выполнение и понимание.
