...
- `Application` — подсчёт оценки, генератор плана, агрегатор профиля развития, проверки ресурсного доступа и аудит;
...
`PlayerDevelopmentService` строит профиль развития непосредственно из завершённых `TeamTrainingAttendance` и `TeamTrainingPlayerResult`. Он одинаково рассчитывает посещаемость, выполнение, понимание, категории, зоны внимания и тренд для игрока, тренера и родителя. Производная статистика не хранится отдельно, поэтому журнал остаётся единственным источником истины.
