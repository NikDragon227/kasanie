...
Дата: 21 августа 2026. Проверены все маршруты `Kasanie.Api/Endpoints`, ролевые политики, запросы EF Core и негативные сценарии с подменой идентификаторов.
...
| TEST-01 | Высокий | `dotnet test` молча пропускал проект без `IsTestProject` | Включён реальный test discovery; 19 тестов выполняются, включая 6 негативных HTTP-сценариев |
...
| `GET /api/coach/players` | Coach | Только активные `CoachPlayerLinks` текущего тренера | Допустимо |
| `GET /api/coach/players/{playerId}` | Coach | `CoachCanAccessAsync`, активная связь | Допустимо; чужой игрок даёт 403 |
...
| `POST /api/admin/users/{id}/invite` | Admin | Перевыпускает ссылку только до установки пароля и только для незаблокированного аккаунта; событие аудируется без токена | Добавлено |
