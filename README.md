...
Production-oriented MVP веб-платформы футбольного развития для школ: команды, тренеры и составы объединены с быстрым журналом тренировок, профилем развития игрока, диагностикой, недельным планом и прогрессом. Профиль автоматически показывает посещаемость, выполнение, понимание, зоны внимания и историю занятий. Владелец школы, тренер, игрок, родитель, региональный аналитик и администратор работают с одной серверной моделью и разными границами доступа.

Параллельно заложен отдельный взрослый публичный контур «Спорт рядом»: поиск игр и совместных тренировок без обязательной регистрации, публичные карточки площадок и событий, запись после входа, вместимость и лист ожидания. Организатор 18+ регистрируется отдельно и не получает фиктивный профиль игрока. Детские данные и школьные кабинеты в этот каталог не экспортируются. Технические границы и план развития: [docs/SPORTS_NEARBY.md](docs/SPORTS_NEARBY.md).
...
Школу и её владельца создают через «Администрирование → Школы». Владелец приглашает тренеров из кабинета школы, создаёт команды и формирует составы. Остальные служебные роли создают через «Администрирование → Пользователи». Система выдаёт одноразовую ссылку на самостоятельную установку пароля. Администратор может выпустить новую ссылку до активации или ссылку сброса для действующего аккаунта; для заблокированной учётной записи операция запрещена. Ссылка действует 24 часа и содержит секретный Identity token — передавайте её только адресату. Вошедший пользователь меняет собственный пароль в разделе «Безопасность». Минимальная длина пароля — 8 символов; обязательны оба регистра, цифра и специальный знак. Для аналитика регион выбирается при приглашении. Пользователи не удаляются физически: администратор блокирует или разблокирует учётную запись.
